...
class KegiatanKabController extends Controller
{
    ...
    public function get_kabupaten_kota(Request $request)
    {
        $provinsi = request('id');
        $data = KabupatenKota::where('id_provinsi',$provinsi)->get();
        $send = "<option selected disabled  value=''>-- Pilih --</option>";
        foreach ($data as $k ) {
            $send .= '<option value="'.$k->id_kota_kab.'">'.$k->nama_kota_kab.'</option>';
        }
    ...
    public function store_pegawai(Request $request)
    {
        ...
        $nip = $data['nip'];
            ...
            return redirect()->route('kegiatan.kabkot.detail',$kegiatan_id);
    }
    
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = Kegiatan::where('kategori','2')->orderByDesc('id')->get();
            ...
            return Datatables::of($data)
                ...
                ->editColumn('aksi', function ($data) {
                    $actionButton = '
                  <a href="'. route('kegiatan.kabkot.detail', $data->id) .'" class="btn waves-effect waves-light btn-warning btn-sm" >
        ...
        return view('admin.kegiatan_kabkot.index',compact('provinsi'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('kegiatan.kabkot.index');
    }
    ...
    public function detail($id_kegiatan)
    {
        ...
        $provinsi = Provinsi::get();
        $kota_kab = KabupatenKota::where('id_provinsi', $data->provinsi_id)->get();
        $detail_pegawai = DetailPegawai::where('kegiatan_id', $id_kegiatan)->get();
        return view('admin.kegiatan_kabkot.detail', compact('data','jenis_pemetaan','provinsi','detail_pegawai','kota_kab'));
    }
