                        ...
                        <div class="container-fluid">
                            <div class="card-header">
                                <div class=" d-flex flex-stack mt-5  ">
                                    <div class="">
                                        <a href="{{ route('kegiatan.provinsi.index') }}" class="btn btn-warning btn-sm mb-3">
                                            <i class="bi bi-arrow-left-short"></i> Kembali</a>
                                    </div>
                                </div>
                            </div>
                            <form method="POST" class="form" action="{{ route('kegiatan.provinsi.update') }}"
                                ...
                                <table id="kt_datatable_zero_configuration"
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="">
                                            ...
                                            <td class="text-left">{{ $item->unker->name }} </td>
                                            ...
                                            <td class="text-left">
                                                ...
                                                <button type="button" class="btn btn-icon btn-light-danger" onclick="alertHapus({{$item->id}})"><i class="bi bi-trash"></i></button>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
            ...
            <div class="modal-content rounded">
                ...
                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                    ...
                    <div class="mb-10">
                        ...
                        <select class="form-select form-select-solid" id="unit_kerja" data-control="select2" data-dropdown-parent="#addPegawai" data-placeholder="Pilih Unit Kerja" data-allow-clear="true">
                            <option></option>
                            @foreach ($unit_kerja as $item)
                            <option value="{{ $item->kunker }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-10">
                        <label for="" class="form-label">Pegawai</label>
                        <select class="form-select form-select-solid" id="pegawai" data-control="select2" data-dropdown-parent="#addPegawai" data-placeholder="Select an option" data-allow-clear="true">
                            <option></option>
                        </select>
                    </div>
                    ...
                    <form method="POST" class="form" action="{{ route('kegiatan.provinsi.pegawai.store') }}"
                        enctype="multipart/form-data">
                        @csrf
    ...
    <div class="modal fade" id="penilaian" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content rounded">
                ...
                <form method="POST" class="form" action="{{ route('kegiatan.provinsi.penilaian.update') }}"
                    enctype="multipart/form-data">
                    @csrf
...
@push('js')
    <script>
        ...
        $("#unit_kerja").change(function() {
            let val = $(this).val();
            $.ajax({
                url: "{{ route('kegiatan.provinsi.pegawai.get') }}",
                method: "GET",
                data: {
                    id: val
                },
        ...
        let count = 0;
            ...
            $.ajax({
                url: "{{ route('kegiatan.provinsi.pegawai.destroy') }}",
                method: "POST",
                data: {
                    _token: _token,
                    id: id
            ...
            var _token = "{{ csrf_token() }}";
            $.ajax({
                url: "{{ route('kegiatan.provinsi.pegawai.get_detail') }}",
                method: "GET",
                data: {
                    _token: _token,
                    id: id
